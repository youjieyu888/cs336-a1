...
class RoPE(torch.nn.Module):
    ...
    def forward(self,
                x: Float[Tensor, "batch_size head sequence_length d_k"],
                token_positions: Int[Tensor, "batch_size sequence_length"]
    ) -> torch.Tensor:
        cos_sel: Float[Tensor, "B S half"] = self.cos_cached.to(dtype=x.dtype, device=x.device)[token_positions].unsqueeze(1)
        sin_sel: Float[Tensor, "B S half"] = self.sin_cached.to(dtype=x.dtype, device=x.device)[token_positions].unsqueeze(1) # B 1 S half
        *prefix, D = x.shape
        half = D//2
...
class Attn(torch.nn.Module):
    ...
    def forward(self,
        ...
        if input_len > 1: # prefill
            ...
            allowed = torch.broadcast_to(valid_k.unsqueeze(1).unsqueeze(2), (valid_k.shape[0], self.num_heads, 1, valid_k.shape[1]))
